...
class UploadsManager{
    ...
    public function folderInfo($folder){
        ...
        $breadcrumbs = $this->breadcrumbs($folder); $slice = array_slice($breadcrumbs, -1); $folderName = current($slice);
        ...
        return compact( 'folder',
            'folderName', 'breadcrumbs', 'subfolders', 'files'
        );
    }
    ...
    protected function fileDetails($path) {
        ...
        return [
            'name' => basename($path),
            'fullPath' => $path,
            'webPath' => $this->fileWebpath($path), 'mimeType' => $this->fileMimeType($path), 'size' => $this->fileSize($path), 'modified' => $this->fileModified($path),
        ];
    }
